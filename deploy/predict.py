...
# cnn_encoder_loaded = torch.jit.script(cnn_encoder_loaded)
...
# lstm_encoder_loaded = torch.jit.script(lstm_encoder_loaded)
...
# attn_decoder_loaded = torch.jit.script(attn_decoder_loaded)
...
from PIL import Image
import numpy as np
# from predict import outer_predict
import copy
import cv2

def run():
    st.title("Image to Latex Generation using CNN + LSTM based encoder and attention based decoder")
    st.header("Markup Generation")
    st.text("Upload a equation image for markup generation")

    uploaded_file = st.file_uploader("Choose a math equation: ", type="png")
    ...
    if uploaded_file is not None:
        ...
        if image.ndim == 2:
          st.image(new_image, caption='Math Equation.', use_column_width=True)
          # print(type(new_image))
        else:
          new_image = new_image[:,:,:1]
          new_image = cv2.resize(new_image, (400,60), interpolation=cv2.INTER_CUBIC)
          st.image(new_image, caption='Math Equation.', use_column_width=True)

        new_image = new_image.astype('float64')
        print(new_image.shape)
        new_image = np.reshape(new_image,(1,60,400))
        st.write("")
        st.write("Recognizing...")
        sentence = outer_predict(new_image)
        st.write("Generated equation is", sentence)
